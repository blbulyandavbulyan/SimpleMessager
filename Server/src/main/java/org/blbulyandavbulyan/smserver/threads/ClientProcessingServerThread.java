            ...
            while (!isTerminated()) {
                try {
                    ...
                    if (!msg.getSender().equals(clientName)) {
                        ...
                        if (msgReceiver.equalsIgnoreCase("SERVER") && msg instanceof ServerCommand) {
                            //todo добавить обработку команды
                            ServerCommand serverCommand = (ServerCommand)msg;
                            serverLogger.info("Пришла команда от " + clientName + ", содержание: " + serverCommand.getCommand());
                            ...
                            if (server.IsClientExists(msgReceiver)) server.getClient(msgReceiver).sendMessage(msg);
                                ...
                                sendMessage(new ServerErrorMessage(clientName, msgReceiver, ServerErrorMessage.ServerErrorCode.MESSAGE_DELIVERY_ERROR_NO_USER_WITH_THIS_NAME));
                        }
                    } else {
                    ...
                    serverLogger.error("Ошибка при работе с клиентом " + clientName, e);
                }
            }
        } catch (SocketException e) {
            if (!(e.getMessage().equals("Socket closed") && isTerminated())) {
                serverLogger.error("Ошибка при работе с сокетом, имя клиента " + clientName, e);
            }
        } catch (EOFException e) {
            serverLogger.info("Пользователь %s отключился.\n".formatted(clientName));
        } catch (IOException e) {
            serverLogger.error("Ошибка ввода вывода при работе с клиентом " + clientName, e);
        } finally {
